...
def send_slack_notification(
    ...
    if fields:
        attachment["fields"] = [
            {"title": field.get("title", ""), "value": field.get("value", ""), "short": field.get("short", True)}
            for field in fields
        ]
...
def format_pipeline_status(
    status: str, sources_fetched: Dict[str, int], duration_seconds: float, errors: Optional[List[str]] = None
) -> Dict:
    """
    ...
    sources_summary = "\n".join([f"• {source}: {count:,} records" for source, count in sources_fetched.items()])
    ...
    return {"severity": severity, "title": title, "message": message, "fields": fields}


def format_error_alert(
    error_type: str, error_message: str, source: Optional[str] = None, traceback: Optional[str] = None
) -> Dict:
    """
    ...
    fields = [{"title": "Error Type", "value": error_type, "short": True}]

    if source:
        fields.append({"title": "Data Source", "value": source, "short": True})

    return {"severity": AlertSeverity.ERROR, "title": title, "message": message, "fields": fields}


def format_data_quality_alert(
    source: str, issue_type: str, details: str, affected_records: int, severity: str = "warning"
) -> Dict:
    """
    ...
    return {"severity": alert_severity, "title": title, "message": message, "fields": fields}
...
def send_error_alert(
    webhook_url: str, error_type: str, error_message: str, source: Optional[str] = None, traceback: Optional[str] = None
) -> bool:
    """
...
def send_data_quality_alert(
    webhook_url: str, source: str, issue_type: str, details: str, affected_records: int, severity: str = "warning"
) -> bool:
    """
