...
def fetch_recent_data():
    ...
    try:
        ...
        print(f"[OK] Fetched {len(chirps_data)} CHIRPS records")
    ...
    try:
        ...
        print(f"[OK] Fetched {len(nasa_data)} NASA POWER records")
    except Exception as e:
    ...
    try:
        ...
        print(f"[OK] Fetched {len(era5_data)} ERA5 records")
    except Exception as e:
    ...
    try:
        ...
        print(f"[OK] Fetched {len(ndvi_data)} NDVI records")
    except Exception as e:
    ...
    try:
        ...
        print(f"[OK] Fetched {len(ocean_data)} Ocean Indices records")
    except Exception as e:
        print(f"[!]  Ocean Indices fetch failed: {e}")
...
def merge_and_store_data(db, data_sources):
    ...
    for _, row in merged.iterrows():
        try:
            ...
            print(f"[!]  Error storing record for {row['date']}: {e}")
            continue
    
    db.commit()
    print(f"\n[OK] Stored {records_stored} climate data records in database")
...
def main():
    ...
    try:
        ...
        if valid_sources == 0:
            print("\n[X] No data fetched from any source")
            return
        
        print(f"\n[OK] Successfully fetched data from {valid_sources}/5 sources")
        
        # Step 2: Merge and store data
        records_stored = merge_and_store_data(db, data_sources)
        
        if records_stored == 0:
            print("\n[X] No data stored in database")
            return
        ...
        if not latest_climate_date:
            print("\n[X] No climate data found in database")
            return
        ...
        for forecast_date in forecast_dates:
            ...
            try:
                ...
                print(f"  [OK] Generated {len(forecasts)} forecasts")
        ...
        print(f"\n[OK] Created {len(recommendations)} recommendations")
        ...
        if high_risk_forecasts:
            print("\n" + "=" * 60)
            print(f"[!]  HIGH-RISK FORECASTS ({len(high_risk_forecasts)} events with >30% probability)")
            print("=" * 60)
            ...
            print("\n[OK] No high-risk forecasts (all probabilities < 30%)")
        
        print("\n" + "=" * 60)
        print("[OK] FORECAST GENERATION COMPLETE!")
        print("=" * 60)
        print(f"\nData Summary:")
        ...
        print(f"\n[X] Error generating forecasts: {e}")
        import traceback
        traceback.print_exc()
        db.rollback()
    finally:
