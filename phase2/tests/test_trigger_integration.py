...
class TestEndToEndCalibration:
    ...
    def historical_data(self):
        ...
        seasonal_multiplier = np.where(
            (months >= 3) & (months <= 5) | (months >= 10) & (months <= 12),
            2.0,  # Rainy seasons
            0.5   # Dry seasons
        )
        ...
        df['spi_30day'] = (
            (df['rainfall_30day'] - df['rainfall_30day'].mean()) / 
            df['rainfall_30day'].std()
        )
        df['spi_90day'] = (
            (df['rainfall_90day'] - df['rainfall_90day'].mean()) / 
            df['rainfall_90day'].std()
        )
        ...
        df['consecutive_dry_days'] = (
            df['is_dry_day']
            .groupby((df['is_dry_day'] != df['is_dry_day'].shift()).cumsum())
            .cumsum()
        )
    ...
    def test_calibrated_thresholds_are_reasonable(self, historical_data):
        ...
        assert 10 < rainfall_stats['daily_rainfall_p95'] < 500, \
            "Daily rainfall 95th percentile outside reasonable range"
        
        assert 20 < rainfall_stats['daily_rainfall_p99'] < 1000, \
    ...
    def test_configuration_file_structure(self):
        ...
        required_keys = [
            'version', 'calibration_date', 'data_period',
            'target_trigger_rates', 'flood_triggers',
            'drought_triggers', 'crop_failure_triggers'
        ]
...
class TestTriggerRateValidation:
    ...
    def test_flood_trigger_rate_within_target(self, processed_data_path):
        ...
        assert 0.05 <= flood_rate <= 0.15, \
            f"Flood trigger rate {flood_rate:.2%} outside target range (5-15%)"
    ...
    def test_drought_trigger_rate_within_target(self, processed_data_path):
        ...
        assert 0.08 <= drought_rate <= 0.20, \
            f"Drought trigger rate {drought_rate:.2%} outside target range (8-20%)"
    ...
    def test_crop_failure_trigger_rate_within_target(self, processed_ndvi_path):
        ...
        assert 0.03 <= crop_rate <= 0.10, \
            f"Crop failure trigger rate {crop_rate:.2%} outside target range (3-10%)"
    ...
    def test_trigger_rates_are_stable(self, processed_data_path):
        ...
        assert rate_std < 0.15, \
            f"Flood trigger rate varies too much across years (std={rate_std:.2%})"
...
class TestFinancialSustainability:
    ...
    def test_combined_trigger_rate_is_sustainable(self, processed_data_path):
        ...
        assert combined_rate < 0.30, \
            f"Combined trigger rate {combined_rate:.2%} too high for financial sustainability"
    ...
    def test_payout_frequency_is_reasonable(self, processed_data_path):
        ...
        df['year'] = pd.to_datetime(df['year'].astype(str) + '-01-01').dt.year \
            if 'year' in df.columns else 2020
        ...
        assert avg_triggers_per_year < 6, \
            f"Average {avg_triggers_per_year:.1f} triggers/year may be unsustainable"
    ...
    def test_thresholds_match_configuration(self):
        ...
        if 'flood_trigger' in df.columns and 'rainfall_7day' in df.columns:
            ...
            if len(high_rainfall) > 0:
                ...
                assert trigger_rate_when_high > 0.5, \
                    "Flood triggers not activating when rainfall exceeds threshold"
        ...
        for key, value in flood_config.items():
            if isinstance(value, dict) and 'threshold' in value:
                assert 'rationale' in value, \
                    f"Threshold {key} missing rationale"
                assert 'data_source' in value, \
                    f"Threshold {key} missing data source"
...
class TestDataConsistency:
    ...
    def test_master_dataset_has_all_triggers(self, master_dataset_path):
        ...
        expected_triggers = [
            'flood_trigger', 'drought_trigger', 'crop_failure_trigger'
        ]
        ...
        for trigger in expected_triggers:
            ...
            assert len(matching_cols) > 0, \
                f"Trigger {trigger} not found in master dataset"
    ...
    def test_no_data_loss_in_merge(self, master_dataset_path):
        ...
        critical_cols = [c for c in df.columns if any(x in c.lower() for x in 
                        ['rainfall', 'temp', 'trigger', 'oni', 'iod'])]
        
        for col in critical_cols:
            if col in missing_pct.index:
                assert missing_pct[col] < 0.50, \
    ...
    def test_temporal_consistency(self, master_dataset_path):
        ...
        df_sorted['date'] = pd.to_datetime(
            ...
            df_sorted['month'].astype(str).str.zfill(2) + '-01'
        )
        
        date_diff = df_sorted['date'].diff()
        # Most gaps should be 1 month (allowing for some missing data)
        expected_gap = pd.Timedelta(days=28)  # Approximately 1 month
...
class TestSeasonalPatterns:
    ...
    def test_flood_triggers_concentrate_in_rainy_seasons(self, processed_data_path):
        ...
        assert rainy_trigger_rate > dry_trigger_rate, \
            "Flood triggers should be more common in rainy seasons"
        
        # At least 60% of flood triggers should occur in rainy seasons
        if df['flood_trigger'].sum() > 0:
            rainy_season_pct = (
                df[df['is_rainy_season']]['flood_trigger'].sum() / 
                df['flood_trigger'].sum()
            )
            assert rainy_season_pct > 0.60, \
                f"Only {rainy_season_pct:.1%} of flood triggers in rainy seasons"
    ...
    def test_drought_triggers_concentrate_in_dry_season(self, processed_data_path):
        ...
        # Calculate trigger rate by season
        dry_trigger_rate = df[df['is_dry_season']]['drought_trigger'].mean()
        wet_trigger_rate = df[~df['is_dry_season']]['drought_trigger'].mean()
        ...
        assert total_drought_rate > 0.05, \
            f"Drought triggers should occur (rate={total_drought_rate:.2%})"
