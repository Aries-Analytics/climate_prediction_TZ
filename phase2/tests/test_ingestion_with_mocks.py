...
from tests.mocks import get_mock_chirps, get_mock_era5, get_mock_nasa_power, get_mock_ndvi, get_mock_ocean_indices
...
def test_mock_chirps_error_simulation():
    ...
    with pytest.raises(Exception) as exc_info:
        mock_api.get_data(start_year=2015, end_year=2020)
...
def test_mock_nasa_power_basic():
    ...
    df = mock_api.get_data(
        latitude=-6.369, longitude=34.889, start_year=2015, end_year=2020, parameters=["T2M", "RH2M"]
    )
...
def test_mock_nasa_power_invalid_parameters():
    ...
    with pytest.raises(Exception):
        mock_api.get_data(
            latitude=-6.369, longitude=34.889, start_year=2015, end_year=2020, parameters=["INVALID_PARAM"]
        )
...
def test_mock_era5_basic():
    ...
    df = mock_api.get_data(start_year=2015, end_year=2020, variables=["temperature_2m", "u_wind_10m"])
...
def test_chirps_ingestion_with_mock(test_db, monkeypatch):
    ...
    def mock_fetch(*args, **kwargs):
        return mock_api.get_data(start_year=kwargs.get("start_year", 2015), end_year=kwargs.get("end_year", 2020))

    try:
        from modules.ingestion.chirps_ingestion import fetch_chirps_data

        monkeypatch.setattr("modules.ingestion.chirps_ingestion._fetch_gee_chirps", mock_fetch)
    ...
    try:
        ...
        with patch("requests.get") as mock_get:
            ...
            mock_response.json.return_value = mock_api.get_http_response(
                latitude=-6.369, longitude=34.889, start_date="20150101", end_date="20201231", parameters="T2M,RH2M"
            )["data"]
            mock_get.return_value = mock_response

            fetch_nasa_power_data(start_year=2015, end_year=2020)
