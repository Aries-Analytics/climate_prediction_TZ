...
class MockNASAPowerAPI(BaseMockAPI):
    ...
    def get_data(
        ...
        df = MockDataGenerator.apply_missing_data(
            df,
            missing_rate=0.005,
            columns=parameters
        )
    ...
    def _generate_temperature(
        ...
        # Add daily variation
        daily_noise = MockDataGenerator.generate_autocorrelated_noise(
            len(dates),
            autocorr=0.8,
            std=1.5
        )
    ...
    def _generate_solar_radiation(
        ...
        solar = MockDataGenerator.generate_seasonal_pattern(
            dates,
            base_value=self.BASE_SOLAR,
            amplitude=self.SOLAR_SEASONAL_AMP,
            peaks=[7, 1]  # Peak in dry seasons
        )
    ...
    def _generate_wind_speed(self, dates: pd.DatetimeIndex) -> np.ndarray:
        ...
        seasonal = MockDataGenerator.generate_seasonal_pattern(
            dates,
            base_value=base_wind,
            amplitude=1.5,
            peaks=[7, 8]  # Windier in dry season
        )
    ...
    def _generate_precipitation(self, dates: pd.DatetimeIndex) -> np.ndarray:
        ...
        rainfall = MockDataGenerator.generate_seasonal_pattern(
            dates,
            base_value=2.0,
            amplitude=4.0,
            peaks=[4, 11]
