...
class MockCHIRPSAPI(BaseMockAPI):
    ...
    def __init__(
        self, fail_rate: float = 0.0, slow_response: bool = False, locations: Optional[List[Dict[str, float]]] = None
    ):
        """
        ...
        self.locations = locations or [
            {"name": "Tanzania", "lat_min": -11.75, "lat_max": -0.99, "lon_min": 29.34, "lon_max": 40.44}
        ]

    def get_data(
        self, start_year: int, end_year: int, bounds: Optional[Dict[str, float]] = None, aggregation: str = "monthly"
    ) -> pd.DataFrame:
        """
    ...
    def _generate_rainfall_pattern(self, dates: pd.DatetimeIndex, location: Dict[str, float]) -> np.ndarray:
        """
        ...
        noise = MockDataGenerator.generate_autocorrelated_noise(len(dates), autocorr=0.6, std=15)
        rainfall = rainfall + noise

        # Add extreme events (droughts and floods)
        rainfall = MockDataGenerator.add_extreme_events(rainfall, event_probability=0.05, event_magnitude=2.0)
    ...
    def get_image_collection(
        ...
        return MockResponseBuilder.build_gee_image_collection(
            df, date_column="date", value_column="rainfall_mm", band_name="precipitation"
        )
...
class MockCHIRPSMultiLocation(MockCHIRPSAPI):
    ...
    CALIBRATED_LOCATIONS = {
        ...
        "Arusha": {"lat_min": -3.5, "lat_max": -3.0, "lon_min": 36.5, "lon_max": 37.0, "rainfall_multiplier": 0.9},
        "Mbeya": {"lat_min": -9.0, "lat_max": -8.5, "lon_min": 32.5, "lon_max": 33.5, "rainfall_multiplier": 1.1},
    }

    def __init__(self, fail_rate: float = 0.0, slow_response: bool = False):
        """Initialize with calibrated locations."""
        locations = [{**loc, "name": name} for name, loc in self.CALIBRATED_LOCATIONS.items()]
        super().__init__(fail_rate, slow_response, locations)

    def get_data_all_locations(self, start_year: int, end_year: int) -> pd.DataFrame:
        """
