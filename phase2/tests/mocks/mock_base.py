...
class MockDataGenerator:
    ...
    def generate_seasonal_pattern(
        dates: pd.DatetimeIndex, base_value: float, amplitude: float, peaks: List[int]
    ) -> np.ndarray:
        """
    ...
    @staticmethod
    def add_noise(values: np.ndarray, noise_std: float, distribution: str = "normal") -> np.ndarray:
        """
    ...
    @staticmethod
    def add_extreme_events(values: np.ndarray, event_probability: float, event_magnitude: float) -> np.ndarray:
        """
        ...
        multipliers = np.where(events, np.random.choice([0.3, 2.5], len(values)), 1.0)  # Drought or flood
        return values * multipliers

    @staticmethod
    def generate_autocorrelated_noise(length: int, autocorr: float = 0.7, std: float = 1.0) -> np.ndarray:
        """
        ...
        for i in range(1, length):
            noise[i] = autocorr * noise[i - 1] + np.random.normal(0, std * np.sqrt(1 - autocorr**2))

        return noise

    @staticmethod
    def create_date_range(start_year: int, end_year: int, freq: str = "MS") -> pd.DatetimeIndex:  # Month start
        """
    ...
    def apply_missing_data(
        df: pd.DataFrame, missing_rate: float = 0.02, columns: Optional[List[str]] = None
    ) -> pd.DataFrame:
        """
...
class MockResponseBuilder:
    ...
    def build_json_response(
        ...
        """
        return {"status_code": status_code, "headers": headers or {"Content-Type": "application/json"}, "data": data}

    @staticmethod
    def build_error_response(
        error_message: str, status_code: int = 500, error_type: str = "InternalServerError"
    ) -> Dict[str, Any]:
        """
        ...
        Returns:
            Mock error response
        """
        return {"status_code": status_code, "error": {"type": error_type, "message": error_message}}

    @staticmethod
    def build_gee_image_collection(
        df: pd.DataFrame, date_column: str = "date", value_column: str = "value", band_name: str = "data"
    ) -> List[Dict[str, Any]]:
        """
            ...
            images.append(
                {
                    ...
                    "bands": [{"id": band_name, "data_type": "float", "crs": "EPSG:4326"}],
