...
from app.services.pipeline import incremental_manager
...
def test_full_pipeline_execution_success(mock_database, pilot_config, mock_all_data_sources):
    ...
    chirps_data = mock_all_data_sources['chirps'].get_data(
        start_year=2024,
        end_year=2024,
        bounds={'min_lat': -8.5, 'max_lat': -7.5, 'min_lon': 36.0, 'max_lon': 37.0}
    )
...
def test_pipeline_fails_when_multiple_sources_fail(mock_database, mock_all_data_sources):
    ...
    with patch.object(mock_all_data_sources['chirps'], 'get_data', side_effect=Exception("Timeout")), \
...
def test_data_quality_validation_detects_anomalies(mock_all_data_sources):
    ...
    with pytest.raises(Exception):
        nasa_data = mock_nasa_with_errors.get_data(
            latitude=-8.0,
            longitude=36.5,
            start_year=2024,
            end_year=2024
...
def test_scheduler_triggers_at_configured_time():
    ...
    config = {
        'schedule': '0 3 * * *',  # 3 AM UTC = 6 AM EAT
