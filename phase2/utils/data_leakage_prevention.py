...
def detect_target_related_features(
    feature_names: List[str], target_name: str, strict: bool = True
    ...
    trigger_patterns = [
        ...
        "moderate_stress_trigger",
        "severe_stress_trigger",
        "trigger_confidence",
        "days_since_trigger",
...
def detect_high_correlation_leakage(X: pd.DataFrame, y: pd.Series, threshold: float = 0.95) -> List[str]:
    """
...
def remove_leaky_features(
    X: pd.DataFrame, target_name: str, y: pd.Series = None, strict: bool = True, correlation_threshold: float = 0.95
    ...
    leaky_by_name, safe_by_name = detect_target_related_features(X.columns.tolist(), target_name, strict=strict)
...
def validate_no_leakage(X: pd.DataFrame, y: pd.Series, target_name: str, max_correlation: float = 0.90) -> bool:
    """
    ...
    leaky_by_name, _ = detect_target_related_features(X.columns.tolist(), target_name, strict=True)
