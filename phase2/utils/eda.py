...
def compare_datasets(df_before, df_after, output_dir="outputs/eda"):
    ...
    if isinstance(df_before, dict):
        ...
        for name, df in df_before.items():
            comparison['before']['datasets'][name] = {
                'shape': df.shape,
                'columns': list(df.columns)
            }
    ...
    if isinstance(df_before, dict):
        comparison['changes'] = {
            ...
            'row_reduction_pct': ((total_rows_before - len(df_after)) / total_rows_before * 100) if total_rows_before > 0 else 0,
    ...
    with open(report_path, 'w') as f:
        ...
        if 'changes' in comparison:
            f.write("CHANGES\n")
            f.write("-" * 80 + "\n")
            f.write(f"Row Reduction: {comparison['changes']['row_reduction']:,} ({comparison['changes']['row_reduction_pct']:.2f}%)\n")
