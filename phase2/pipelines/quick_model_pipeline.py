...
def log_experiment(experiment_id: str, results: dict, output_dir: Path) -> None:
    ...
    with open(log_file, "a") as f:
        json.dump({"experiment_id": experiment_id, "timestamp": datetime.now().isoformat(), **results}, f)
        f.write("\n")
...
def main():
    ...
    try:
        ...
        if lstm_valid_mask.sum() > 0:
            ...
            print(
                f"    ✓ Valid predictions: {lstm_valid_mask.sum()}/{len(y_test)} (first {12-1} samples are NaN padding)"
            )
        else:
    ...
    print(f"  {'-'*50}")
    print(
        f"  {'Baseline':<15} {baseline_test_metrics['r2_score']:<10.3f} {baseline_test_metrics['rmse']:<10.2f} {'---':<15}"
    )
    ...
    for model_name, result in models_results.items():
        if result is not None:
            ...
            print(
                f"  {model_name:<15} {r2:<10.3f} {rmse:<10.2f} {improvement:+.3f} ({improvement/max(abs(baseline_test_metrics['r2_score']), 0.001)*100:+.1f}%)"
            )
    ...
    try:
        ...
        seasonal_results = evaluate_by_season(
            y_true=y_test, y_pred=y_pred_test, dates=test_dates, target_name="rainfall"
        )
    ...
    try:
        ...
        summary = {
            ...
            "improvement_over_baseline": {
                ...
                "r2_improvement_pct": (test_metrics["r2_score"] - baseline_test_metrics["r2_score"])
                / max(abs(baseline_test_metrics["r2_score"]), 0.001)
                * 100,
                "rmse_improvement_pct": (baseline_test_metrics["rmse"] - test_metrics["rmse"])
