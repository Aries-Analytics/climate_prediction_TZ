...
- **SHADOW_RUN_START date guard (Apr 14, 2026):** Orchestrator Stage 5, auto_log_service, and alert_service all filter forecast_log queries to `>= date(2026, 4, 14)`. Prevents manual backfills or stale data from inflating the day count and triggering the final report prematurely. Post-shadow-run evaluation (Stage 4 → Stage 5 → generate_final_report) is fully zone-aware and automatic.
