...
- **12 forecasts/day instead of 24 — stale ingestion modules (Apr 15):** Docker-compose mounts `./modules:/app/modules:ro` for the scheduler container, but the two-zone PILOT_LOCATIONS update (commit `6a4f32b`) only updated `backend/modules/ingestion/` — the top-level `phase2/modules/ingestion/` was left with hardcoded Morogoro coordinates (-6.8211, 37.6595). All 5 ingestion sources stored climate data at the wrong location; `forecast_service.py` found no climate data within ±0.01° of the actual pilot zone coordinates, so only 1 of the 8 DB locations had enough data to generate forecasts. Fix: updated all 5 files in `phase2/modules/ingestion/` with the PILOT_LOCATIONS loop. **Architectural debt:** two copies of the same ingestion code (`phase2/modules/` and `phase2/backend/modules/`) — must be consolidated (see Q2 roadmap).
