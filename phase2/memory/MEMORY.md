...
- **CI lint is now enforced (Apr 16, commit `f03701f`):** Before this date, CI reported green on every commit but the three lint jobs (flake8/black/isort) had `continue-on-error: true` AND were referencing deleted paths (`modules/`, `utils/`) — effectively swallowing 521 violations silently. Fixed: paths updated to `backend/modules/ backend/utils/ tests/`, `continue-on-error` removed. All 521 violations resolved (bulk via black/autoflake, manual for bare-except and unused-var cases). CI now genuinely gates code quality. Also removed inactive duplicate workflow at `phase2/.github/workflows/` (GitHub Actions only reads the repo-root `.github/`).
- **args/persona_config.yaml trimmed (Apr 16):** Only `forbidden_patterns` and `test_allowed_patterns` are read by Python code (`scripts/audit.py`). All other fields (`pilot_location_id`, `pilot_region`, `current_phase`, `active_persona`, `sprint_phase`, `expected_feature_count`, `feature_schema_path`, `active_model_config`, etc.) were unread scaffolding — drift bait that still said `pilot_location_id: 6` and `pilot_region: Morogoro`. Removed. Contract constants live in code (`risk_service.py`, `shadow_run.py`, `active_model.json`).
- **Persona system is aspirational / not runtime-wired (Apr 16):** Orchestrator / Backend Architect / Frontend Engineer / Auditor is a clean separation of concerns mental model, but it is NOT mechanically enforced — no automation switches personas or loads different context per persona. `CLAUDE.md` at the repo root is the single active context file auto-loaded on every session. SKILL.md + handover_prompt.md updated to reflect this honestly. If the persona system is ever revived, rebuild from clean rather than maintaining stale scaffolding.
- **state.json is aspirational / does not exist (Apr 16):** Previous docs (SKILL.md, handover_prompt.md, goals/) referenced `state.json → shared_contract` as "canonical source of truth" but the file has never existed in the repo. References removed from context docs and handover prompt. If a session state file is ever needed, design it as a proper module, not an implicit schema.
...
- Commits (morning): `aeebfc9` (tz contract), `99db863` (pd.Timestamp boundary), `5294641` (session log + MEMORY), `51fac02` (Day 1 complete), `647cd92` (gap policy), `3be8296` (projected end date), `bbe0a04` (end-of-day memory sync)
- **Harness / CI cleanup (afternoon):** Swept all repo folders (.claude/commands, .agent/rules, .github/workflows, args, context, hardprompts). Found 5 classes of stale content: deprecated `/log-run` command, duplicate-source-of-truth fields in `persona_config.yaml`, stale `state.json` references in multiple docs, wrong pilot constants in 3 context files, broken CI paths pointing to deleted `modules/`/`utils/` directories with `continue-on-error: true` hiding 521 violations. Executed Path B: fixed CI paths, removed `continue-on-error`, deleted duplicate workflow, trimmed persona_config to audit-only, refreshed 5 doc files, fixed all 521 lint violations (black/autoflake auto-fix for most, manual for E722/F841). CI now genuinely enforces lint.
- Commits (afternoon): `5826e8f` (remove /log-run), `f03701f` (Path B + harness cleanup + 521 lint fixes)
