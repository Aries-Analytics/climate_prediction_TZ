...
from utils.logger import get_logger, log_error, log_info, log_warning, setup_logging
...
def process_location_source(location_name: str, loc_data: dict, source: str, start_year: int, end_year: int) -> tuple:
    ...
    bounds = {"lat_min": lat - 0.25, "lat_max": lat + 0.25, "lon_min": lon - 0.25, "lon_max": lon + 0.25}

    try:
        df = None
        count = 0
        if source == "NASA_POWER":
            df = fetch_nasa_power_data(latitude=lat, longitude=lon, start_year=start_year, end_year=end_year)

        elif source == "CHIRPS":
            df = fetch_chirps_data(start_year=start_year, end_year=end_year, bounds=bounds, use_gee=True)

        elif source == "ERA5":
            df = fetch_era5_data(latitude=lat, longitude=lon, start_year=start_year, end_year=end_year)

        elif source == "NDVI":
            df = fetch_ndvi_data(start_year=start_year, end_year=end_year, bounds=bounds, use_gee=True)
        ...
        result = {"location": location_name, "source": source, "status": "SUCCESS", "records": count}
        return result, df

    except Exception as e:
        log_error(f"[{location_name}] {source} Failed: {str(e)}")
        result = {"location": location_name, "source": source, "status": "FAILED", "error": str(e)}
        return result, None
...
def run_orchestrator(sources: Optional[List[str]] = None, parallel: bool = False):
    ...
    if parallel:
        ...
        with ThreadPoolExecutor(max_workers=3) as executor:  # Reduced workers to avoid API limits
            ...
            for loc_name, loc_data in locations.items():
                for source in loc_sources:
                    futures.append(
                        executor.submit(
                            process_location_source, loc_data["name"], loc_data, source, start_year, end_year
                        )
                    )
        ...
        for loc_name, loc_data in locations.items():
            for source in loc_sources:
                res, df = process_location_source(loc_data["name"], loc_data, source, start_year, end_year)
                results.append(res)
                if df is not None:
    ...
    for r in results:
        ...
        details = (
            f"{r.get('records', 0)} records" if r["status"] == "SUCCESS" else f"Error: {r.get('error', 'Unknown')}"
        )
