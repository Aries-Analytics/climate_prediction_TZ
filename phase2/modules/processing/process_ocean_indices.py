...
def _add_climate_risk_indicators(df):
    ...
    df["climate_risk_class"] = pd.cut(
        df["overall_climate_risk"], bins=[0, 25, 50, 75, 100], labels=["low", "moderate", "high", "extreme"]
    )
...
def _add_insurance_triggers(df):
    ...
    if "drought_risk_score" in df.columns and "flood_risk_score" in df.columns:
        df["trigger_severity"] = np.maximum(df["drought_risk_score"] / 100, df["flood_risk_score"] / 100)
...
def _calculate_climate_uncertainty(df):
    ...
    strong_consistent = (df["enso_intensity"] > 1.0) & (df["iod_intensity"] > 0.6) & (df["conflicting_signals"] == 0)
    uncertainty[strong_consistent] = 0.2

    # High uncertainty: Weak or conflicting signals
    weak_or_conflicting = (df["enso_intensity"] < 0.5) | (df["iod_intensity"] < 0.3) | (df["conflicting_signals"] == 1)
...
def _calculate_forecast_confidence(df):
    ...
    low_conf = (df["combined_intensity"] < 0.5) | ((df["enso_phase_change"] == 1) | (df["iod_phase_change"] == 1))
