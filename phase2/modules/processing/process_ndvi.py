...
def _add_ndvi_temporal_stats(df):
    ...
    df["ndvi_trend_30day"] = (
        df["ndvi"].rolling(window=30, min_periods=10).apply(lambda x: _calculate_trend(x), raw=True)
    )
...
def _add_vegetation_condition_index(df):
    ...
    # VCI classification
    df["vci_class"] = pd.cut(
        df["vci"], bins=[0, 20, 35, 50, 100], labels=["extreme_stress", "severe_stress", "moderate_stress", "normal"]
    )
...
def _add_drought_stress_indicators(df):
    ...
    df["is_stressed"] = ((df["ndvi"] < 0.3) | (df["ndvi_anomaly_std"] < -1.0) | (df["vci"] < 35)).astype(int)

    # Stress duration (consecutive stressed periods)
    df["stress_duration"] = (
        df.groupby((df["is_stressed"] != df["is_stressed"].shift()).cumsum())["is_stressed"].cumsum()
        * df["is_stressed"]
    )

    # Severe stress indicator (multiple criteria)
    df["is_severe_stress"] = ((df["ndvi"] < 0.2) | (df["ndvi_anomaly_std"] < -1.5) | (df["vci"] < 20)).astype(int)

    # Severe stress duration
    df["severe_stress_duration"] = (
        df.groupby((df["is_severe_stress"] != df["is_severe_stress"].shift()).cumsum())["is_severe_stress"].cumsum()
        * df["is_severe_stress"]
    )
    ...
    df["is_recovering"] = (
        (df["is_stressed"] == 0) & (df["is_stressed"].shift(1) == 1) & (df["ndvi_trend_30day"] > 0)
    ).astype(int)
...
def _add_growth_stage_indicators(df):
    ...
    df["is_senescence"] = ((df["ndvi_trend_30day"] < 0) & (df["ndvi"] < df["ndvi_clim_mean"])).astype(int)

    # Critical growth period (high NDVI with positive trend)
    df["is_critical_period"] = ((df["ndvi"] > 0.4) & (df["ndvi_trend_30day"] > 0) & (df["vci"] > 35)).astype(int)
...
def _add_crop_failure_risk(df):
    ...
    df["crop_failure_risk_class"] = pd.cut(
        df["crop_failure_risk"], bins=[0, 25, 50, 75, 100], labels=["low", "moderate", "high", "extreme"]
    )
...
def _add_insurance_triggers(df):
    ...
    log_info(
        f"Using crop failure thresholds: VCI={vci_critical}, VCI_duration={vci_duration}d, "
    ...
    df["crop_failure_trigger"] = (
        (df["vci"] < vci_critical) | (df["ndvi_anomaly_std"] < ndvi_anomaly_threshold)
    ).astype(int)
    ...
    ndvi_signal = (
        (df["ndvi_anomaly_std"] < ndvi_anomaly_threshold) & (df["stress_duration"] >= ndvi_anomaly_duration)
    ).astype(float)
    ...
    df["crop_failure_trigger_confidence"] = (
        vci_signal * 0.30 + ndvi_signal * 0.30 + risk_signal * 0.25 + percentile_signal * 0.15
    )
    ...
    df["severe_stress_trigger"] = (
        (df["vci"] < vci_critical) | (df["severe_stress_duration"] >= severe_stress_duration)
    ).astype(int)
