...
def generate_calibrated_config(
    ...
    flood_thresholds = calibrate_flood_triggers(
        rainfall_stats,
        df_chirps,
        target_trigger_rate=target_flood_rate
    )
    
    # Calibrate drought triggers
    log_info("\n--- Calibrating Drought Triggers ---")
    drought_thresholds = calibrate_drought_triggers(
        drought_stats,
        df_chirps,
        target_trigger_rate=target_drought_rate
    )
    ...
    crop_failure_thresholds = calibrate_crop_failure_triggers(
        vegetation_stats,
        df_ndvi,
        target_trigger_rate=target_crop_failure_rate
    )
    ...
    with open(output_file, 'w', encoding='utf-8') as f:
        ...
        f.write(f"#   Flood: {flood_thresholds['achieved_trigger_rate']*100:.2f}% (target: {target_flood_rate*100:.1f}%)\n")
        f.write(f"#   Drought: {drought_thresholds['achieved_trigger_rate']*100:.2f}% (target: {target_drought_rate*100:.1f}%)\n")
        f.write(f"#   Crop Failure: {crop_failure_thresholds['achieved_trigger_rate']*100:.2f}% (target: {target_crop_failure_rate*100:.1f}%)\n")
        f.write("#\n")
    ...
    log_info(f"  Flood: {flood_thresholds['achieved_trigger_rate']*100:.2f}% (target: {target_flood_rate*100:.1f}%, confidence: {flood_thresholds['confidence_score']:.3f})")
    log_info(f"  Drought: {drought_thresholds['achieved_trigger_rate']*100:.2f}% (target: {target_drought_rate*100:.1f}%, confidence: {drought_thresholds['confidence_score']:.3f})")
    log_info(f"  Crop Failure: {crop_failure_thresholds['achieved_trigger_rate']*100:.2f}% (target: {target_crop_failure_rate*100:.1f}%, confidence: {crop_failure_thresholds['confidence_score']:.3f})")
    log_info("\n" + "=" * 80)
