...
def analyze_rainfall_distribution(df: pd.DataFrame) -> Dict[str, float]:
    ...
    log_info(f"Data quality: {valid_records}/{total_records} valid records "
...
def analyze_drought_indicators(df: pd.DataFrame) -> Dict[str, float]:
    ...
    # Validate required columns
    required_cols = ['spi_30day', 'spi_90day', 'consecutive_dry_days', 
                     'rainfall_deficit_mm', 'rainfall_deficit_pct', 'month']
    missing_cols = [col for col in required_cols if col not in df.columns]
    if missing_cols:
        error_msg = f"Missing required columns: {missing_cols}"
        log_error(error_msg)
    ...
    for spi_col in ['spi_30day', 'spi_90day']:
        ...
        log_info(f"    Moderate drought (SPI < -1.0): {moderate_drought} ({results[f'{spi_col}_moderate_drought_pct']:.1f}%)")
        log_info(f"    Severe drought (SPI < -1.5): {severe_drought} ({results[f'{spi_col}_severe_drought_pct']:.1f}%)")
        log_info(f"    Extreme drought (SPI < -2.0): {extreme_drought} ({results[f'{spi_col}_extreme_drought_pct']:.1f}%)")
    ...
    if len(positive_deficit_mm) > 0:
        ...
        log_info(f"  Mean deficit: {results['rainfall_deficit_mm_mean']:.2f}mm ({results['rainfall_deficit_pct_mean']:.1f}%)")
        log_info(f"  Median deficit: {results['rainfall_deficit_mm_median']:.2f}mm ({results['rainfall_deficit_pct_median']:.1f}%)")
        log_info(f"  95th percentile: {results['rainfall_deficit_mm_p95']:.2f}mm ({results['rainfall_deficit_pct_p95']:.1f}%)")
...
def analyze_vegetation_stress(df: pd.DataFrame) -> Dict[str, float]:
    ...
    # Validate required columns
    required_cols = ['vci', 'ndvi', 'ndvi_anomaly_std', 'stress_duration', 
                     'severe_stress_duration', 'crop_failure_risk']
    missing_cols = [col for col in required_cols if col not in df.columns]
    if missing_cols:
        error_msg = f"Missing required columns: {missing_cols}"
        log_error(error_msg)
...
def generate_threshold_report(
    ...
    output_dir: str = "data/outputs/calibration"
) -> str:
    """
    ...
    if 'rainfall' in analysis_results:
        ...
        if extreme_rain_pct < 5 or extreme_rain_pct > 15:
            overall_warnings.append(
                f"Extreme rainfall events ({extreme_rain_pct:.1f}%) outside target range (5-15%) - "
                "adjust thresholds"
            )
    ...
    if 'drought' in analysis_results:
        ...
        if drought_pct < 8 or drought_pct > 20:
            overall_warnings.append(
                f"Drought trigger rate ({drought_pct:.1f}%) outside target range (8-20%) - "
                "adjust thresholds"
            )
