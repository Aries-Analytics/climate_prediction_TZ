...
def calibrate_flood_triggers(
    ...
    log_info(f"Acceptable range: {(target_trigger_rate-tolerance)*100:.1f}% - {(target_trigger_rate+tolerance)*100:.1f}%")
...
def calibrate_drought_triggers(
    ...
    for iteration in range(max_iterations):
        df_test = df.copy()
        df_test['season'] = df_test['month'].apply(
            lambda m: 'wet' if m in [10, 11, 12, 1, 2, 3, 4, 5] else 'dry'
        )
        df_test['dry_day_threshold'] = df_test['season'].map({
            'wet': wet_season_dry_days,
            'dry': dry_season_dry_days
        })
        df_test['drought_trigger'] = (
            (df_test['spi_30day'] < spi_threshold) &
            (df_test['consecutive_dry_days'] >= df_test['dry_day_threshold'])
        ).astype(int)
...
def calibrate_crop_failure_triggers(
    ...
    for iteration in range(max_iterations):
        ...
        df_test['crop_failure_trigger'] = (
            ...
            ((df_test['ndvi_anomaly_std'] < ndvi_anomaly_threshold) & (df_test['stress_duration'] >= ndvi_anomaly_duration)) |
...
def simulate_trigger_rates(
    ...
    if 'drought_triggers' in thresholds or 'spi_30day_threshold' in thresholds:
        ...
        df_drought['season'] = df_drought['month'].apply(
            lambda m: 'wet' if m in [10, 11, 12, 1, 2, 3, 4, 5] else 'dry'
        )
        df_drought['dry_day_threshold'] = df_drought['season'].map({
            'wet': wet_dry_days,
            'dry': dry_dry_days
        })
    ...
    if 'crop_failure_triggers' in thresholds or 'vci_threshold' in thresholds:
        ...
        df_crop['crop_failure_trigger'] = (
            ...
            ((df_crop['ndvi_anomaly_std'] < ndvi_anomaly_threshold) & (df_crop['stress_duration'] >= ndvi_anomaly_duration)) |
    ...
    if 'flood_trigger_rate' in results and 'drought_trigger_rate' in results and 'crop_failure_trigger_rate' in results:
        ...
        overall_rate = min(1.0, results['flood_trigger_rate'] + results['drought_trigger_rate'] + results['crop_failure_trigger_rate'])
