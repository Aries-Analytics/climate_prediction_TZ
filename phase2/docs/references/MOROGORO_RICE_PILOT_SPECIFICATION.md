# Morogoro Rice Pilot Specification

**Document Version:** 1.0  
**Created:** January 12, 2026  
**Location:** Morogoro Region, Kilombero Basin, Tanzania  
**Purpose:** Single-location pilot to validate ensemble climate prediction model before multi-location rollout

---

## Executive Summary

The **Morogoro Rice Pilot** is a focused, single-location parametric insurance pilot program designed to validate the production-ready ensemble climate prediction model in a real-world agricultural setting. 

**Model Selection**: The ensemble Random Forest model achieved **98.3% R² score** on the overall test set (2020-2023 data, source: `FINAL_MODEL_PIPELINE_REPORT.md`), demonstrating excellent generalization across all locations.

**Location Selection**: Morogoro was selected based on **superior location-specific model performance** (R² = 0.855 in spatial cross-validation, **the highest among all 6 locations**, source: `6_LOCATION_EXPANSION_SUMMARY.md`).
...
| **Overall Ensemble Model** | 0.983 (98.3%) | Test set accuracy (2020-2023, all locations combined) |
...
> - **98.3% (Overall ensemble)**: How well the model performs on the held-out test set (2020-2023) when trained on all available data. This validates the overall model architecture.
...
|--------------|--------|-----------|---------------|--------|
| **Rainfall Deficit (Drought)** | Cumulative rainfall | < 150mm in 30 days | Vegetative/Reproductive | $60/farmer |
| **Excessive Rainfall (Flood)** | Cumulative rainfall | > 400mm in 30 days | Any stage | $75/farmer |
...
## Data Sources (Morogoro-Specific)

The ensemble model uses 5 integrated data sources for Morogoro:

| Source | Variables | Resolution | Coverage |
|--------|-----------|------------|----------|
| **CHIRPS** | Rainfall | 0.05° (~5km) | 2000-Present |
| **NASA POWER** | Temperature, Solar, Humidity | 0.5° (~50km) | 2000-Present |
| **ERA5** | Pressure, Wind, Soil | 0.25° (~25km) | 2000-Present |
| **MODIS NDVI** | Vegetation Index | 250m | 2000-Present |
| **Ocean Indices** | ENSO, IOD, NAO | Global | 2000-Present |

**Total Features:** 162 numeric features engineered from these sources  
...
1. **Model Validation**: Verify 98.3% R² accuracy translates to real-world forecast reliability
...
- "Morogoro Rice Pilot (1,000 Farmers)"
- "Single-Location Pilot - Kilombero Basin"
...
### Phase 1: Morogoro Pilot (Current)
**Duration:** 12 months  
**Scope:** 1 location, 1,000 farmers  
**Objective:** Validate model and operations
...
**Next Review:** July 2026 (6-month pilot checkpoint)
