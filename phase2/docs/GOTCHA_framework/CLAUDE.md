...
* **Database Destruction Prevention (CRITICAL)** — NEVER run `docker compose down`, `docker compose down --remove-orphans`, `docker compose down -v`, `docker volume prune`, or `docker system prune` on ANY production or staging server. These commands destroy database volumes and cause irreversible data loss. To deploy changes: (1) `docker compose build <service>`, (2) `docker compose up -d <service>` — target ONLY the changed service. NEVER tear down the full stack to fix a single service. Before ANY deployment touching DB, backend, or docker-compose: run `/opt/hewasense/backup_db.sh` and verify the backup exists. Automated daily backup runs at 05:00 EAT to `/opt/hewasense/backups/` with 30-day retention. Full policy: `phase2/DATA_PROTECTION_POLICY.md`. This guardrail exists because on 2026-04-24, `docker compose down --remove-orphans` destroyed 9 days of live shadow run pipeline data.
...
| Pipeline / scheduler change | `docs/references/DATA_PIPELINE_REFERENCE.md` · `docs/AUTOMATED_PIPELINE_DEPLOYMENT.md` · `memory/MEMORY.md` · `DATA_PROTECTION_POLICY.md` (if deployment/backup rules affected) |
