...
monthly_stats = df.groupby("month").agg(
    {"temp_mean_c": "mean", "rainfall_mm": "mean", "ndvi": "mean", "humidity_pct": "mean"}
)
...
axes[0, 0].boxplot(
    [rainfall_by_enso.get("La Niña", []), rainfall_by_enso.get("Neutral", []), rainfall_by_enso.get("El Niño", [])],
...
axes[0, 1].boxplot(
    [temp_by_enso.get("La Niña", []), temp_by_enso.get("Neutral", []), temp_by_enso.get("El Niño", [])],
    labels=["La Niña", "Neutral", "El Niño"],
    patch_artist=True,
)
...
axes[1, 0].boxplot(
    [ndvi_by_enso.get("La Niña", []), ndvi_by_enso.get("Neutral", []), ndvi_by_enso.get("El Niño", [])],
    labels=["La Niña", "Neutral", "El Niño"],
    patch_artist=True,
)
axes[1, 0].set_ylabel("NDVI", fontweight="bold")
...
for phase in df["enso_phase_name"].unique():
    ...
    axes[1, 1].scatter(
        phase_data["date"], phase_data["oni"], label=phase, color=colors.get(phase, "gray"), s=100, alpha=0.7
    )
...
axes[0].scatter(
    df["rainfall_mm"], df["ndvi"], c=df["month"], cmap="viridis", s=100, alpha=0.6, edgecolors="black", linewidth=0.5
)
z = np.polyfit(df["rainfall_mm"], df["ndvi"], 1)
p = np.poly1d(z)
axes[0].plot(df["rainfall_mm"], p(df["rainfall_mm"]), "r--", linewidth=2, label=f"y={z[0]:.4f}x+{z[1]:.3f}")
...
if len(valid_data) > 0:
    ...
    axes[1].plot(
        valid_data["rainfall_lag1"],
        p_lag(valid_data["rainfall_lag1"]),
        "r--",
        linewidth=2,
        label=f"y={z_lag[0]:.4f}x+{z_lag[1]:.3f}",
    )
...
fig.suptitle("Tanzania Climate Data Summary Dashboard (2020-2021)", fontsize=18, fontweight="bold")
