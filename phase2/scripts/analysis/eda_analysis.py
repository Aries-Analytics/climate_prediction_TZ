...
if all(col in df.columns for col in ["rainfall_mm", "iod"]):
    # Classify IOD
    df["iod_phase"] = pd.cut(df["iod"], bins=[-np.inf, -0.4, 0.4, np.inf], labels=["Negative", "Neutral", "Positive"])
...
missing_df = pd.DataFrame(
    {"Column": missing_summary.index, "Missing": missing_summary.values, "Percent": missing_pct.values}
)
...
if "rainfall_mm" in df.columns:
    ...
    if len(extreme_rain) > 0:
        print(
            f"  Max: {extreme_rain['rainfall_mm'].max():.2f} mm in {extreme_rain.loc[extreme_rain['rainfall_mm'].idxmax(), 'year']}-{extreme_rain.loc[extreme_rain['rainfall_mm'].idxmax(), 'month']:02d}"
        )
    ...
    if len(drought_rain) > 0:
        print(
            f"  Min: {drought_rain['rainfall_mm'].min():.2f} mm in {drought_rain.loc[drought_rain['rainfall_mm'].idxmin(), 'year']}-{drought_rain.loc[drought_rain['rainfall_mm'].idxmin(), 'month']:02d}"
        )
...
if "temp_mean_c" in df.columns:
    ...
    if len(hot_months) > 0:
        print(
            f"  Max: {hot_months['temp_mean_c'].max():.2f}°C in {hot_months.loc[hot_months['temp_mean_c'].idxmax(), 'year']}-{hot_months.loc[hot_months['temp_mean_c'].idxmax(), 'month']:02d}"
        )
