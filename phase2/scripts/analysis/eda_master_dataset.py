...
focus_vars = ['rainfall_mm', 'ndvi', 'temp_mean_c', 'oni', 'iod', 'humidity_pct',
              'rainfall_anomaly_mm', 'ndvi_anomaly', 'drought_severity', 'flood_risk_score_left']
...
if 'enso_phase' in df.columns:
    ...
    phase_counts.plot(kind='bar', stacked=True, ax=axes[0, 0], 
...
if 'oni' in df.columns and 'rainfall_mm' in df.columns:
    scatter = axes[0, 1].scatter(df['oni'], df['rainfall_mm'], 
...
if 'iod' in df.columns and 'ndvi' in df.columns:
    scatter = axes[1, 1].scatter(df['iod'], df['ndvi'], 
                                 c=df['year'], cmap='viridis', alpha=0.6, s=30)
...
if 'rainfall_anomaly_mm' in df.columns:
    ...
    axes[1, 0].fill_between(df['date'], 0, df['rainfall_anomaly_mm'], 
                            where=df['rainfall_anomaly_mm']>0, color='blue', alpha=0.3, label='Above normal')
    axes[1, 0].fill_between(df['date'], 0, df['rainfall_anomaly_mm'], 
                            where=df['rainfall_anomaly_mm']<0, color='red', alpha=0.3, label='Below normal')
...
if 'ndvi_anomaly' in df.columns:
    ...
    axes[1, 1].fill_between(df['date'], 0, df['ndvi_anomaly'], 
                            where=df['ndvi_anomaly']>0, color='green', alpha=0.3, label='Above normal')
    axes[1, 1].fill_between(df['date'], 0, df['ndvi_anomaly'], 
                            where=df['ndvi_anomaly']<0, color='brown', alpha=0.3, label='Below normal')
...
if 'rainfall_mm' in df.columns:
    report.append(f"\nRAINFALL EXTREMES:")
    report.append(f"  Wettest month: {df['rainfall_mm'].max():.2f} mm ({df.loc[df['rainfall_mm'].idxmax(), 'year']}-{df.loc[df['rainfall_mm'].idxmax(), 'month']:02d})")
    report.append(f"  Driest month: {df['rainfall_mm'].min():.2f} mm ({df.loc[df['rainfall_mm'].idxmin(), 'year']}-{df.loc[df['rainfall_mm'].idxmin(), 'month']:02d})")

if 'ndvi' in df.columns:
    report.append(f"\nVEGETATION EXTREMES:")
    report.append(f"  Highest NDVI: {df['ndvi'].max():.4f} ({df.loc[df['ndvi'].idxmax(), 'year']}-{df.loc[df['ndvi'].idxmax(), 'month']:02d})")
    report.append(f"  Lowest NDVI: {df['ndvi'].min():.4f} ({df.loc[df['ndvi'].idxmin(), 'year']}-{df.loc[df['ndvi'].idxmin(), 'month']:02d})")
