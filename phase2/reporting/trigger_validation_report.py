    ...
    def _analyze_confidence_scores(
        ...
        for col in confidence_cols:
            ...
            results[trigger_type] = {
                ...
                'improvement': round(new_scores.mean() - old_scores.mean(), 3) if len(old_scores) > 0 and len(new_scores) > 0 else 0
            }
...
"""
        ...
        for _, row in comparison_df.iterrows():
            ...
            report += f"{row['rate_change']:+.1f}% | {row['target_rate_min']}-{row['target_rate_max']}% | {status_icon} |\n"
        ...
        if 'seasonal_analysis' in seasonal_results:
            ...
            if 'flood' in seasonal_results['seasonal_analysis']:
                ...
                report += f"- **Events in Rainy Season:** {flood_data['rainy_season_count']} / {flood_data['total_count']}\n"
            ...
            if 'drought' in seasonal_results['seasonal_analysis']:
                ...
                report += f"- **Events in Dry Season:** {drought_data['dry_season_count']} / {drought_data['total_count']}\n"
        ...
        report += f"${financial_results['payout_reduction']:,.2f} ({financial_results['payout_reduction_pct']:+.1f}%) |\n"
        ...
        if all_within_target and sustainability and seasonal_score >= 65:
            report += "The trigger recalibration has been **successful**. All triggers now operate within acceptable ranges, "
...
def main():
    ...
    parser.add_argument('--old-data', type=str, required=True,
                       help='Path to old processed data CSV')
    parser.add_argument('--new-data', type=str, required=True,
                       help='Path to new processed data CSV')
