...
"""
        ...
        if flood_old > 15 and 5 <= flood_new <= 15:
            improvements.append(f"✓ **Flood trigger rate reduced** from {flood_old:.1f}% to {flood_new:.1f}% (now within 5-15% target)")
        ...
        if crop_old < 3 and 3 <= crop_new <= 10:
            improvements.append(f"✓ **Crop failure trigger activated** - increased from {crop_old:.1f}% to {crop_new:.1f}% (now within 3-10% target)")
        
        # Financial improvement
        payout_reduction = old_metrics['annual_payout'] - new_metrics['annual_payout']
        if payout_reduction > 0:
            improvements.append(f"✓ **Annual payout reduced** by ${payout_reduction:,.2f} ({payout_reduction / old_metrics['annual_payout'] * 100:.1f}%)")
        ...
        if new_loss_ratio < 80 and old_loss_ratio >= 80:
            improvements.append(f"✓ **Financial sustainability achieved** - loss ratio reduced from {old_loss_ratio:.1f}% to {new_loss_ratio:.1f}%")
        ...
        all_within_target = (
            5 <= new_metrics.get('flood_trigger_rate', 0) <= 15 and
            8 <= new_metrics.get('drought_trigger_rate', 0) <= 20 and
            3 <= new_metrics.get('crop_failure_trigger_rate', 0) <= 10
        )
