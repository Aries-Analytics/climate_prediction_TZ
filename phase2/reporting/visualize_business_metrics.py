...
class BusinessMetricsVisualizer:
    ...
    def _plot_alert_distribution(self):
        ...
        ax1.pie(alert_counts.values, labels=alert_counts.index, autopct="%1.1f%%", colors=colors, startangle=90)
        ax1.set_title("Alert Type Distribution", fontsize=14, fontweight="bold")

        # Severity distribution
        severity_counts = df["severity"].value_counts()
        ax2.bar(severity_counts.index, severity_counts.values, color=["green", "orange", "red"], alpha=0.7)
    ...
    def _plot_risk_heatmap(self):
        ...
        fig, ax = plt.subplots(figsize=(14, 6))
        sns.heatmap(risk_matrix, annot=True, fmt="d", cmap="YlOrRd", cbar_kws={"label": "Number of Triggers"}, ax=ax)

        ax.set_title("Risk Heatmap: Trigger Events by Month and Year", fontsize=16, fontweight="bold")
        ax.set_xlabel("Month", fontsize=12)
