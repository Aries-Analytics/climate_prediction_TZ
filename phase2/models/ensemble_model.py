...
class EnsembleModel(BaseModel):
    ...
    def __init__(self, model_name: str = "ensemble", custom_config: Optional[Dict[str, Any]] = None):
        """
    ...
    def _validate_weights(self) -> None:
        ...
        logger.info(
            f"Ensemble weights: RF={self.weights['rf']:.2f}, "
    ...
    def train(
        ...
        """
        logger.warning(
            "Ensemble model doesn't train directly. "
        ...
        if self.method == "weighted_average":
            ...
            if len(invalid_indices) > 0:
                ...
                ensemble_pred[invalid_indices] = (
                    rf_weight_norm * rf_pred[invalid_indices] + xgb_weight_norm * xgb_pred[invalid_indices]
                )

                logger.debug(f"Used RF+XGB only for {len(invalid_indices)} samples " f"(LSTM sequence padding)")
