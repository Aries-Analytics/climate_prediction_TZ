...
def train_all_models(
    ...
    results = {
        ...
        "data_shapes": {"train": X_train.shape, "val": X_val.shape, "test": X_test.shape},
        "models": {},
    }
    ...
    if train_ensemble and len(trained_models) >= 3:
        ...
        try:
            ...
            # Load base models
            ensemble_model.load_base_models(trained_models["rf"], trained_models["xgb"], trained_models["lstm"])
    ...
    for model_name, model_results in results["models"].items():
        if "error" in model_results:
            ...
            if "error" not in metrics:
                logger.info(
                    f"{model_name.upper()}: R²={metrics['r2']:.4f}, "
...
def save_training_results(results: Dict[str, Any], save_dir: str, experiment_name: Optional[str] = None) -> str:
    """
...
def load_trained_models(models_dir: str) -> Dict[str, Any]:
    ...
    if ensemble_path.exists() and len(loaded_models) >= 3:
        ...
        ensemble_model.load_base_models(loaded_models["rf"], loaded_models["xgb"], loaded_models["lstm"])
