...
def calculate_quantile_predictions(
    models: List[Any], X: np.ndarray, quantiles: List[float] = [0.1, 0.5, 0.9]
) -> Dict[str, np.ndarray]:
    """
...
def calculate_prediction_intervals(y_pred: np.ndarray, y_true: np.ndarray, confidence: float = 0.95) -> Dict[str, Any]:
    """
...
def save_predictions_with_uncertainty(
    y_true: np.ndarray, y_pred: np.ndarray, intervals: Dict[str, Any], save_path: str
) -> str:
    """
...
def evaluate_by_region(y_true: np.ndarray, y_pred: np.ndarray, regions: np.ndarray) -> Dict[str, Dict[str, float]]:
    """
    ...
    for region in unique_regions:
        ...
        if mask.sum() > 0:
            ...
            logger.info(f"  {region}: R²={region_metrics['r2']:.4f}, " f"RMSE={region_metrics['rmse']:.4f}")
...
def evaluate_by_season(y_true: np.ndarray, y_pred: np.ndarray, months: np.ndarray) -> Dict[str, Dict[str, float]]:
    """
...
def plot_predictions_vs_actual(
    y_true: np.ndarray, y_pred: np.ndarray, save_path: str, title: str = "Predictions vs Actual"
) -> str:
    """
...
def plot_feature_importance(
    importance_df: pd.DataFrame, save_path: str, top_n: int = 20, title: str = "Feature Importance"
) -> str:
    """
...
def generate_evaluation_report(
    ...
    plot_predictions_vs_actual(y_test, y_pred, str(viz_path), title=f"{model_name.upper()}: Predictions vs Actual")
    viz_files.append(str(viz_path))

    # 2. Residuals over time
    viz_path = output_path / f"{model_name}_residuals_over_time.png"
    plot_residuals_over_time(y_test, y_pred, None, str(viz_path), title=f"{model_name.upper()}: Residuals Over Time")
    viz_files.append(str(viz_path))
    ...
    if hasattr(model, "get_feature_importance"):
        try:
            ...
            plot_feature_importance(importance_df, str(viz_path), title=f"{model_name.upper()}: Feature Importance")
            viz_files.append(str(viz_path))
